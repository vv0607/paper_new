...
from .sfd_head_utils import GAF, CPConvs, Baseline_color, ColorEh
...
class SFDHead(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        # self.colorbase_layer = Baseline_color()
        self.colorbase_layer = ColorEh()
        ...
        c0 = self.model_cfg.ROI_AWARE_POOL.NUM_FEATURES_RAW   #102
        c1 = self.model_cfg.ROI_AWARE_POOL.NUM_FEATURES       #128
        ...
        # )
        self.conv_pseudo = spconv.SparseSequential(
            block(c0, c0, 3, padding=1, indice_key='rcnn_subm1', conv_type='subm'),
            # block(c0, c0, 3, padding=1, indice_key='rcnn_subm1', conv_type='subm'),
            block(c0, c1, 3, stride=2, padding=1, indice_key='rcnn_spconv1', conv_type='spconv'),
            # block(c1, c1, 3, padding=1, indice_key='rcnn_subm2', conv_type='subm'),
            block(c1, c1, 3, padding=1, indice_key='rcnn_subm2', conv_type='subm')
        )
    ...
    def forward(self, batch_dict):
        ...
        points_features_expand = self.colorbase_layer(points_features, points_neighbor)[1:]
    ...
    def get_box_cls_layer_loss(self, forward_ret_dict, iou_target):
        ...
        if loss_cfgs.CLS_LOSS == 'BinaryCrossEntropy':
            ...
            # batch_loss_cls = F.binary_cross_entropy_with_logits(torch.sigmoid(rcnn_cls_flat), rcnn_cls_labels.float(), reduction='none')
    ...
    def get_box_reg_layer_loss_valid(self, forward_ret_dict):
        ...
        if loss_cfgs.REG_LOSS_VALID == 'smooth-l1':
            ...
            # rcnn_reg = rcnn_reg.to(torch.float32)
            # reg_targets_valid = reg_targets_valid.to(torch.float32)
