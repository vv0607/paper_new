...
def cal_coiou_3d(box3d1: torch.Tensor, box3d2: torch.Tensor, enclosing_type: str = "smallest"):
    """calculated 3d COIoU loss. assume the 3d bounding boxes are only rotated around z axis
    ...
    Returns:
        (torch.Tensor): (B, N) 3d COIoU loss
        (torch.Tensor): (B, N) 3d IoU
    """
    iou3d, corners1, corners2, z_range, u3d = cal_iou_3d(box3d1, box3d2, verbose=True)
    ...
    ff = 1e-5/torch.exp(1.6+1/torch.atan(abs(theta_box1-theta_box2)/2+0.1))
    d2 = x_offset * x_offset + y_offset * y_offset + z_offset * z_offset
    c2 = w * w + h * h + z_range * z_range
    v = (4 / 3.14 ** 2) * abs(torch.atan(w_box1/h_box1) - torch.atan(w_box2/h_box2))
    # # print('v:', v)
    alpha = v / (v - iou3d + (1 + 1e-7))
    # wandb.log({'alpha*v':alpha*v*1e-3})
    # coiou = 1.0 - iou3d + d2/c2 + alpha*v +ff
    coiou = 1.0 - iou3d + d2 / c2
...
if __name__ == "__main__":
    ...
    coiou_loss, iou = cal_coiou_3d(tensor1, tensor1)
    print(coiou_loss)
    print(iou)
