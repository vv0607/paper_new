import torch
import numpy as np
from pcdet.ops.iou3d.box_intersection_2d import oriented_box_intersection_2d
from pcdet.ops.iou3d.min_enclosing_box import smallest_bounding_box
...
def cal_iou(box1: torch.Tensor, box2: torch.Tensor):
    ...
    # print('corners1 & corners2', corners1,corners2)
...
def cal_coiou_3d(box3d1: torch.Tensor, box3d2: torch.Tensor, enclosing_type: str = "smallest"):
    """calculated 3d DIoU loss. assume the 3d bounding boxes are only rotated around z axis
    ...
    y_offset = box3d1[..., 1] - box3d2[..., 1]
    z_offset = box3d1[..., 2] - box3d2[..., 2]
    w_box1, h_box1 =[box3d1[..., 3],box3d1[..., 4]]
    w_box2, h_box2 = [box3d2[..., 3], box3d2[..., 4]]
    theta_box1, theta_box2 = [box3d1[..., 6], box3d2[..., 6]]
    d2 = x_offset * x_offset + y_offset * y_offset + z_offset * z_offset
    c2 = w * w + h * h + z_range * z_range
    # diou = 1. - iou3d + d2 / c2
    v = (4 / 3.14 ** 2) * abs(torch.atan(w_box1/h_box1) - torch.atan(w_box2/h_box2))

    print('v:', v)
    with torch.no_grad():
        alpha = v / (v - iou3d + (1 + 1e-7))
    print('alpha*v', alpha*v*40)
    coiou= 1- iou3d + (d2/c2*20 + alpha*v*40)
    print('d2 / c2', d2 / c2*20)
    return coiou, iou3d
...
box3d1 = np.array([0.23, 0.2, -0.01, 3.6, 1.6,1.44, -0.13])    # (x,y,z,w,h,l,alpha)
box3d2 = np.array([0.13, 0.2, -0.01, 3.5, 1.6,1.44, -0.15])
...
# giou_loss, iou3d = cal_giou_3d(tensor1, tensor2)
diou_loss, iou3d = cal_diou_3d(tensor1, tensor2)
coiou_loss, iou3d = cal_coiou_3d(tensor1, tensor2)

print('iou3d',iou3d)
print('diou:', diou_loss)
print('coiou:', coiou_loss)

# box1 = np.array([0.14, 0.2, 3.6, 1.6, -0.13])
# box2 = np.array([0.13, 0.24, 3.58, 1.63, -0.14])
# tensor1 = torch.FloatTensor(box1).unsqueeze(0).unsqueeze(0).cuda()
# tensor2 = torch.FloatTensor(box2).unsqueeze(0).unsqueeze(0).cuda()
#
# iou2d,_,_,_= cal_iou(tensor1, tensor2)
# print('iou2d',iou2d)